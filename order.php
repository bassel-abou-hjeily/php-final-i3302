...
if (isset($_SESSION['user_id'])) {
    ...
    cart_products.id AS cart_id, cart_products.quantity, cart_products.status ,
      cart_products.date_order, cart_products.price as newPrice, cart_products.discount as newDiscount,
    ...
    AND  cart_products.status ='order' 
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<td>' . '<img src="images/' . $row['category_name'] . '/' . $row['Picture'] . '" alt="Image" width="60px" height="60px">'  . '</td>' .
                ...
                "</td>" . "<td>" . $row["Price"] . '$' . "</td>" . "<td>" . $row["newDiscount"] . '%' . "</td>" .
